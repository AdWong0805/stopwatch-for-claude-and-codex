...
void AppCodexMicro::onRunning()
{
    ...
    // A+B stays local. It also works while disconnected so the Usage page
    // (Wi-Fi only) is reachable without a BLE host.
    const bool toggle_page = input::hasKeyEvent(event, input::KeyEvent::TogglePage);
...
const char* AppCodexMicro::debugScreenName()
{
    ...
    switch (_view->currentPage()) {
        ...
        case view::CodexMicroView::Page::Agent:
            return "agent";
        case view::CodexMicroView::Page::Usage:
        default:
            return "usage";
    }
}
