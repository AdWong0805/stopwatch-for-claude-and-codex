...
constexpr uint32_t UsageTrack              = 0x2A2F2C;
constexpr uint32_t UsageMuted              = 0x8A918D;
...
void CodexMicroView::init(lv_obj_t* parent)
{
    ...
    renderUsage(_page_roots[static_cast<std::size_t>(Page::Usage)]);
...
void CodexMicroView::usageActionEvent(lv_event_t* event)
{
    ...
    const std::size_t slot = static_cast<std::size_t>(context->icon);
    if (slot >= UsageActions.size()) {
        return;
    }
    audio::play_tone(1200, FeedbackToneDurationSeconds, FeedbackToneVolume);
    GetHAL().vibrate(FeedbackVibrationDurationMs, FeedbackVibrationStrength);
    usage_link::GetUsageLink().requestAction("claude", UsageActions[slot]);
}

void CodexMicroView::renderUsage(lv_obj_t* parent)
{
    lv_obj_t* title = lv_label_create(parent);
    lv_label_set_text(title, "USAGE");
    lv_obj_set_style_text_font(title, &lv_font_montserrat_16, LV_PART_MAIN);
    lv_obj_set_style_text_color(title, lv_color_hex(UsageMuted), LV_PART_MAIN);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 34);

    _usage_status_label = lv_label_create(parent);
    lv_label_set_text(_usage_status_label, "");
    lv_obj_set_style_text_font(_usage_status_label, &lv_font_montserrat_14, LV_PART_MAIN);
    lv_obj_set_style_text_color(_usage_status_label, lv_color_hex(UsageMuted), LV_PART_MAIN);
    lv_obj_align(_usage_status_label, LV_ALIGN_TOP_MID, 0, 58);
    ...
    for (std::size_t index = 0; index < 2; ++index) {
        ...
        lv_obj_set_style_arc_color(arc, lv_color_hex(UsageTrack), LV_PART_MAIN);
        ...
        lv_obj_align(pct, LV_ALIGN_TOP_MID, ArcX[index] + ArcSize / 2 - 233, ArcY + 52);

        lv_obj_t* reset            = lv_label_create(parent);
        _usage_reset_labels[index] = reset;
        lv_label_set_text(reset, "");
        lv_obj_set_style_text_font(reset, &lv_font_montserrat_14, LV_PART_MAIN);
        lv_obj_set_style_text_color(reset, lv_color_hex(UsageMuted), LV_PART_MAIN);
        lv_obj_align(reset, LV_ALIGN_TOP_MID, ArcX[index] + ArcSize / 2 - 233, ArcY + 88);
        ...
        lv_obj_align(name, LV_ALIGN_TOP_MID, ArcX[index] + ArcSize / 2 - 233, ArcY + ArcSize + 8);

        lv_obj_t* track = lv_obj_create(parent);
        lv_obj_remove_flag(track, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_size(track, ArcSize - 30, 8);
        lv_obj_set_pos(track, ArcX[index] + 15, ArcY + ArcSize + 34);
        stylePanel(track, UsageTrack, UsageTrack, 4, 0);

        lv_obj_t* fill          = lv_obj_create(track);
        _usage_week_bars[index] = fill;
        lv_obj_remove_flag(fill, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_set_pos(fill, 0, 0);
        lv_obj_set_size(fill, 2, 8);
        stylePanel(fill, AccentColors[index], AccentColors[index], 4, 0);

        lv_obj_t* week            = lv_label_create(parent);
        _usage_week_labels[index] = week;
        lv_label_set_text(week, "WK --");
        lv_obj_set_style_text_font(week, &lv_font_montserrat_14, LV_PART_MAIN);
        lv_obj_set_style_text_color(week, lv_color_hex(UsageMuted), LV_PART_MAIN);
        lv_obj_align(week, LV_ALIGN_TOP_MID, ArcX[index] + ArcSize / 2 - 233, ArcY + ArcSize + 50);
    }

    // Claude quick actions relayed through the companion program.
    ...
    for (std::size_t index = 0; index < ButtonNames.size(); ++index) {
        ...
        stylePanel(button, Touch, TouchBorder, 24, 1);
        lv_obj_set_style_bg_color(button, lv_color_hex(0x2A2F2C), PressedStyle);
        lv_obj_set_style_border_color(button, lv_color_hex(ClaudeOrange), PressedStyle);
        // IconContext.icon carries the action slot for the shared callback.
        _usage_action_contexts[index] = {.owner = this, .icon = static_cast<Icon>(index)};
        lv_obj_add_event_cb(button, usageActionEvent, LV_EVENT_CLICKED, &_usage_action_contexts[index]);
...
void CodexMicroView::updateUsagePage()
{
    ...
    if (_usage_status_label != nullptr) {
        ...
        lv_label_set_text(_usage_status_label, status);
    }
    ...
    for (std::size_t index = 0; index < 2; ++index) {
        ...
        if (_usage_arcs[index] != nullptr) {
            lv_arc_set_value(_usage_arcs[index], session.valid ? session.percent : 0);
        }
        if (_usage_pct_labels[index] != nullptr) {
            char text[8] = {};
            if (session.valid) {
            ...
            lv_label_set_text(_usage_pct_labels[index], text);
        }
        if (_usage_reset_labels[index] != nullptr) {
            lv_label_set_text(_usage_reset_labels[index],
                              session.valid && session.reset[0] != '\0' ? session.reset : "");
        }
        if (_usage_week_bars[index] != nullptr) {
            const int width = week.valid ? std::max(2, TrackWidth * week.percent / 100) : 2;
            lv_obj_set_width(_usage_week_bars[index], width);
        }
        if (_usage_week_labels[index] != nullptr) {
            char text[16] = {};
            if (week.valid) {
            ...
            lv_label_set_text(_usage_week_labels[index], text);
        }
    }
}
...
void CodexMicroView::renderPage()
{
    ...
    ESP_LOGI(Tag, "page=%s", _page == Page::Command ? "command" : (_page == Page::Agent ? "agent" : "usage"));
}
...
void CodexMicroView::togglePage()
{
    if (!_functional_enabled) {
        // The Usage page needs Wi-Fi only, so it stays reachable while the
        // BLE host is absent: A+B toggles between Pairing and Usage.
        if (_pairing_screen == nullptr) {
            return;
        }
        _usage_offline_view = !_usage_offline_view;
        if (_usage_offline_view) {
...
bool CodexMicroView::ready() const
{
    ...
    for (lv_obj_t* const page_root : _page_roots) {
...
void CodexMicroView::update(const CodexMicroState& state)
{
    ...
    if (_page == Page::Usage && (_functional_enabled || _usage_offline_view)) {
        const bool usage_refresh_due = tick - _usage_last_update_tick >= 500;
        if (_page_dirty || usage_refresh_due) {
            updateUsagePage();
            _usage_last_update_tick = tick;
        }
    }
    _page_dirty          = false;
