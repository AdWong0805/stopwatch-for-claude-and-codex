...
void SerialDebug::handleLine(char* line)
{
    ...
    if (std::strcmp(command, "wifi") == 0) {
        ...
        if (ssid == nullptr) {
            result("wifi", "FAIL", "reason=usage_debug_wifi_ssid_password");
            return;
        }
        usage_link::GetUsageLink().setWifiConfig(ssid, password == nullptr ? "" : password);
        result("wifi", "PASS", "action=saved note=reboot_to_apply");
        return;
    }
    if (std::strcmp(command, "host") == 0) {
        ...
        if (host == nullptr) {
            result("host", "FAIL", "reason=usage_debug_host_ip_port");
            return;
        }
        usage_link::GetUsageLink().setHostConfig(host, static_cast<uint16_t>(parseUnsigned(port, 8787, 1, 65535)));
        result("host", "PASS", "action=saved note=reboot_to_apply");
        return;
    }
    if (std::strcmp(command, "usage") == 0) {
        ...
        if (sub != nullptr && std::strcmp(sub, "clear") == 0) {
            ...
            result("usage", "PASS", "action=config_cleared note=reboot_to_apply");
            return;
        }
...
void SerialDebug::printHelp()
{
    ...
    std::printf("DBG HELP commands=wifi_[ssid]_[password],host_[ip]_[port],usage,usage_clear\r\n");
